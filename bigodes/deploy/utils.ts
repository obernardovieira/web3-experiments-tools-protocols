...
export const verifyEnoughBalance = async (wallet: Wallet, amount: BigNumber) => {
  // Check if the wallet has enough balance
  const balance = await wallet.getBalance();
  // if (balance < amount) throw `⛔️ Wallet balance is too low! Required ${ethers.utils.formatEther(amount)} ETH, but current ${wallet.address} balance is ${ethers.utils.formatEther(balance)} ETH`;
}
...
export const deployContract = async (contractArtifactName: string, constructorArguments?: any[], options?: DeployContractOptions) => {
  ...
  log(`Estimated deployment cost: ${ethers.utils.formatEther(deploymentFee)} ETH`);
